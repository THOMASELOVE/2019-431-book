...
Suppose we wanted to look only at those observations (subjects) whose Age is at least 21 and at most 79. Suppose also that we want to look at a few other variables of interest, not just Age, Height and Sex, but some other things as well. To start, we'll do the following:
...
4. Select the variables we will be using in the remainder of this chapter.
...
```{r nh_data_2179_bmibysexhist-fig}
ggplot(data = nh_data_2179, aes(x = BMI, fill = Sex)) + 
    geom_histogram(color = "white", bins = 20) + 
    labs(title = "Histogram of BMI by Sex for NHANES subjects ages 21-79",
         x = "Body-mass index") + 
    guides(fill = FALSE) +
    facet_grid(Sex ~ .)
```

As an accompanying numerical summary, we might ask how many people fall into each of these Sex categories, and what is their "average" BMI.
...
We can compare the distribution of BMI across `Race1` groups, as well.

```{r nh_data_2179_bmibyrace-fig}
ggplot(data = nh_data_2179, aes(x = Race1, y = BMI, fill = Race1)) + 
    geom_boxplot() +
    labs(title = "BMI by Race1 for NHANES ages 21-79",
         x = "Body-mass index") + 
    guides(fill = FALSE) 
```    
...
### BMI by Diabetes and General Health Status

We'll build a panel of boxplots to try to understand the relationships between BMI, General Health Status and Diabetes. Note the use of `coord_flip` to rotate the graph 90 degrees, and the use of `labeller` within `facet_wrap` to include both the name of the (Diabetes) variable and its value.
...
Here's a plot of faceted histograms, which might be used to address similar questions related to the relationship between Overall Health, BMI and Sex.

```{r nh_data_2179_bmibyhealthbysex2-fig}
ggplot(data = nh_data_2179, aes(x = BMI, fill = Sex)) + 
    geom_histogram(color = "white", bins = 20) + 
    labs(title = "BMI by Sex, Overall Health for NHANES ages 21-79",
         x = "Body-mass index") + 
    guides(fill = FALSE) +
