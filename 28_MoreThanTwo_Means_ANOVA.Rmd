# Comparing 3 or more Population Means: Analysis of Variance {#ANOVA}

Recall the National Youth Fitness Survey, which we explored a small piece of in some detail back in Section \@ref(NYFS-Study). We'll look at a different part of the same survey here - specifically the 280 children whose data are captured in the `nyfs2` file.
...
```{r hers2_numerical_by_income, message=FALSE, warning=FALSE}
...
So, in this case, at our usual 5% significance level, since the *p* value for the Kruskal-Wallis test is 0.31, we'd conclude that we cannot reject H~0~ and can only conclude that there is no significant difference between the true mean GMQ scores across the three income categories. Again, note that this isn't much help, though, because we don't have any measure of effect size, nor do we have any confidence intervals.
...
Sure. There are both ANOVA and ANCOVA analogues using the bootstrap, and in fact, there are power calculations based on the bootstrap, too. If you want to see some example code, look at https://sammancuso.com/2015/05/28/bootstrap-anova-with-pairwise-post-hoc-contrasts/
...
## The Proportion of Variance Explained by the Factor, or $\eta^2$
...
H~0~: $\mu_{Low} = \mu_{Middle} = \mu_{High}$ vs. H~A~: H~0~ not true.
...
### Pairwise Comparisons of Group Means using Tukey's Honestly Significant Differences
...
```{r revised plot hsd for glucose}
