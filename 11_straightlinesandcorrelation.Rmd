...
```{r}
s1 %>% select(SEQN, sex, race_eth, age_child, protein, fat) %>% knitr::kable()
```
...
p1 + p2 + 
    plot_annotation(title = "Impact of adjusting loess smooth span, in NNYFS data")
```
...
Another way we'll frequently summarize a model's coefficient is to use the `broom` package's `tidy` function to put the coefficient estimates into a tibble.

```{r}
broom::tidy(lm(fat ~ protein, data = nnyfs),
