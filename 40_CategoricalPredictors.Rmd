...
```{r kitchen sink model revised 2}
...
So, again, here's our model, now pandered into a prettier format using the `pander` function from the `pander` package.

```{r kitchen model again}
ebmodel.1 <- lm(bmi ~ age + gender + employed + married + 
                    alcohol + education, data = emp_bmi_noNA)
pander::pander(ebmodel.1)
```
...
```{r pairs_scatter_mat_emp_bmi_fig, fig.height = 6}
...
```{r ggpairs_emp_bmi_fig, message = FALSE, fig.height = 6}
